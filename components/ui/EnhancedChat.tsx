...
export default function EnhancedAIChatWidget() {
  ...
  useEffect(() => {
    if (isOpen && messages.length === 0) {
      setMessages([{
        role: 'assistant',
        content: 'Hi! I\'m Mohamed\'s AI assistant. I can help you learn about his skills, projects, and experience. What would you like to know?',
        timestamp: new Date(),
        responseType: 'greeting',
        actionSuggestions: ['Tell me about Mohamed\'s skills', 'Show me his projects', 'What\'s his experience?']
      }]);
    }
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      const errorMessage: ChatMessage = {
        role: 'assistant',
        content: 'Sorry, I\'m having trouble right now. Please try again later or contact Mohamed directly.',
        timestamp: new Date()
      };
      ...
      {isOpen && (
        <div className="fixed bottom-6 right-6 z-50 w-80 h-96 bg-white dark:bg-gray-800 rounded-xl shadow-2xl border border-gray-200 dark:border-gray-700 flex flex-col">
          ...
          <div className="bg-gradient-to-r from-blue-500 to-purple-600 text-white p-4 rounded-t-xl flex justify-between items-center">
            <div>
              <h3 className="font-bold text-sm">🤖 Mohamed&apos;s AI Assistant</h3>
              <p className="text-xs opacity-90">Ask me anything about Mohamed!</p>
            </div>
            <button
